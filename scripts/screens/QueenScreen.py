...
class QueenScreen(Screens):
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame_gui.UI_BUTTON_START_PRESS:
            if event.ui_element == self.back_button:
                ...
                game.patrolled.append(self.queens[self.selected_queen].ID)
                output = Cat.mediate_relationship(
                    self.queens[self.selected_queen], self.selected_cat_1, self.selected_cat_2,
                    self.allow_romantic)
    ...
    def screen_switches(self):
        ...
        for cat in Cat.all_cats_list:
            if cat.status in ["queen", "queen's apprentice"] and not (cat.dead or cat.outside):
                self.queens.append(cat)

        self.page = 1

        if self.queens:
            if Cat.fetch_cat(game.switches["cat"]) in self.queens:
                self.selected_queen = self.queens.index(Cat.fetch_cat(game.switches["cat"]))
            else:
                self.selected_queen = 0
        else:
            self.selected_queen = None
    ...
    def update_mediator_info(self):
        ...
        if self.selected_queen is not None:  # It can be zero, so we must test for not None here.
            x_value = 630
            mediator = self.queens[self.selected_queen]
            ...
            mediator_number = len(self.queens)
            if self.selected_queen < mediator_number - 1:
                self.next_med.enable()
            else:
                self.next_med.disable()

            if self.selected_queen > 0:
    ...
    def update_list_cats(self):
        self.all_cats_list = [i for i in Cat.all_cats_list if
                              (i.ID != self.queens[self.selected_queen].ID) and not (i.dead or i.outside) and i.moons >= 14]
    ...
    def update_buttons(self):
        ...
        if self.selected_queen is not None:
            if self.queens[self.selected_queen].not_working():
                invalid_mediator = True
                error_message += "This queen can't work this moon. "
            elif self.queens[self.selected_queen].ID in game.patrolled:
                invalid_mediator = True
                error_message += "This queen has already worked this moon. "
        else:
            invalid_mediator = True
